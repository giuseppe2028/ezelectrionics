...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("role").isString().isIn(["Manager", "Customer", "Admin"]), //the request body must contain an attribute named "role", the attribute must be a string and its value must be one of the three allowed options
        ...
        this.router.get(
            ...
            (_req: any, res: any, next: any) => this.controller.getUsers()
        ...
        this.router.delete(
            "/",
            this.authService.isAdmin,
            (_req: any, res: any, next: any) => this.controller.deleteAll()
                .then(() => res.status(200).end())
                .catch((err: any) => next(err))
        )
        ...
        this.router.patch(
            ...
            body("birthdate").isDate({format : "YYYY-MM-DD"}).bail() // Bail early so that invalid formats throw a validation exception
