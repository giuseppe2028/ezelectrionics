...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("category").isString().notEmpty().isIn(Object.values(Category)),
            body("quantity").isInt({gt : 0}).toInt(),
            body("details").isString().optional(),
            body("sellingPrice").isFloat({gt : 0}).toFloat(),
            oneOf([
                body("arrivalDate").isDate({format : "YYYY-MM-DD"}).bail() // Bail early so that invalid formats throw a validation exception
                    .isBefore().withMessage(() => { throw new DateError }),
                body("arrivalDate").isEmpty().customSanitizer(() => null) // Convert empty date to null
            ]),
        ...
        this.router.patch(
            ...
            body("quantity").isInt({gt: 0}).toInt(),
            oneOf([
                body("changeDate").isDate({format : "YYYY-MM-DD"}).bail() // Bail early so that invalid formats throw a validation exception
                    .isBefore().withMessage(() => { throw new DateError }),
                body("changeDate").isEmpty().customSanitizer(() => null) // Convert empty date to null
            ]),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
        ...
        this.router.patch(
            ...
            body("quantity").isInt({gt: 0}).toInt(),
            oneOf([
                body("sellingDate").isDate({format : "YYYY-MM-DD"}).bail() // Bail early so that invalid formats throw a validation exception
                    .isBefore().withMessage(() => { throw new DateError }),
                body("sellingDate").isEmpty().customSanitizer(() => null) // Convert empty date to null
            ]),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
                this.controller.sellProduct(
                    req.params.model,
                    req.body.quantity,
        ...
        this.router.get(
            ...
            query("category").if(query("grouping").isEmpty()).isEmpty(),
            query("category").if(query("grouping").equals("category")).isString().notEmpty().isIn(Object.values(Category)),
            query("category").if(query("grouping").equals("model")).isEmpty(),
            query("model").if(query("grouping").isEmpty()).isEmpty(),
            query("model").if(query("grouping").equals("model")).isString().notEmpty(),
            query("model").if(query("grouping").equals("category")).isEmpty(),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
