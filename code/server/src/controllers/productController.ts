...
import { DateIncompatibleWithProductArrivalError, EmptyProductStockError, LowProductStockError } from "../errors/productError";
...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null): Promise<void> {
        return this.dao.createProduct(
            ...
            details,
            sellingPrice,
            arrivalDate ?? Utility.getDateString()
        )
    }
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> {
        ...
        changeDate = changeDate ?? Utility.getDateString()
        if (new Date(changeDate) < new Date(product.arrivalDate))
            throw new DateIncompatibleWithProductArrivalError
    ...
    async sellProduct(model: string, quantity: number, sellingDate: string | null): Promise<number> {
        const product = await this.dao.getProductByModel(model)
        sellingDate = sellingDate ?? Utility.getDateString()
        if (new Date(sellingDate) < new Date(product.arrivalDate))
            throw new DateIncompatibleWithProductArrivalError
        if (product.quantity == 0)
            throw new EmptyProductStockError
        else if(product.quantity < quantity)
            throw new LowProductStockError
        return this.dao.updateProductQuantity(model, -quantity, sellingDate)
    }
     ...
     */
    async getProducts(grouping:  "category" | "model" | null, category: string | null, model: string | null): Promise<Product[]> {
        switch (grouping) {
            case "category": {
                return this.dao.getProductsByCategory(category)
            }
            case "model": {
                return Promise.all([this.dao.getProductByModel(model)])
            }
            default: {
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null): Promise<Product[]> {
        switch (grouping) {
            ...
            default: {
                return this.dao.getProducts(true)
            }
        }
    }
