...
import { UserNotAdminError, UserIsAdminError } from "../errors/userError";
...
class UserController {
    ...
    async getUserByUsername(user: User, username: string) :Promise<User> {
        // if admin wants to get a user, or user wants to get themselves => get 
    ...
    async deleteUser(user: User, username: string) :Promise<Boolean> {
        //if a user wants to delete an other user
        if (user.username != username){
            //if the user is not an admin => error not an admin
            if (!Utility.isAdmin(user)) throw new UserNotAdminError

            const deleteuser = await this.dao.getUserByUsername(username)
            //if the user is an amin and wants to delete an other admin => error user is admin
            if (Utility.isAdmin(deleteuser)) throw new UserIsAdminError

        } 
        //if a user wants to delete himself
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string) :Promise<User> { 
        //if a user wants to edit an other user
        if (user.username != username){
            //if the user is not an admin => error not an admin
            if (!Utility.isAdmin(user)) throw new UserNotAdminError

            const deleteuser = await this.dao.getUserByUsername(username)
            //if the user is an amin and wants to edit an other admin => error user is admin
            if (Utility.isAdmin(deleteuser)) throw new UserIsAdminError

        } 
        //if a user wants to edit himself
